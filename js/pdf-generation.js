...
// Calculate total content height for vertical centering - UPDATED with product links
function calculateTotalContentHeight(pdf, maxWidth) {
    ...
    if (pattern.product_tearsheet_url && pattern.product_tearsheet_url.trim()) productLinksCount++;
    if (pattern.product_page_url && pattern.product_page_url.trim()) productLinksCount++;
    if (pattern.product_360_url && pattern.product_360_url.trim()) productLinksCount++;
    patternDetailsLines += productLinksCount;
